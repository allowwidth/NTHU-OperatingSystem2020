...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
        DEBUG('z',"[E] Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID()  << "] is now selected for execution, thread [" << this->getID() << "] is replaced, and it has executed [" << this->BurstTime << "] ticks");
...
Thread::Sleep (bool finishing)
{
    ...
    this->BurstTime += (kernel->stats->totalTicks - this->StartTime); 
    if(!finishing){
        DEBUG('z',"[D] Tick [" << kernel->stats->totalTicks << "]: Thread [" << this->getID()  << "] update approximate burst time, from: [" << this->predictTime << "], add [" << this->BurstTime << "], to [" << 0.5*this->predictTime + 0.5*this->BurstTime << "]");
