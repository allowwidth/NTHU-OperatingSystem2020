...
void Machine::Run()
{
	...
	if (debug->IsEnabled('m')) // machine emulation
	{
...
void Machine::OneInstruction(Instruction *instr)
{
	...
	if (!ReadMem(registers[PCReg], 4, &raw))
		return; // exception occurred (an exception is an event that occurs
	...
	if (debug->IsEnabled('m'))
	{
		...
		sprintf(buf, str->format, TypeToReg(str->args[0], instr),
				TypeToReg(str->args[1], instr), TypeToReg(str->args[2], instr)); // a sprintf function is used to write a formatted
																				 // string to character string buffer
		cout << "\t" << buf << "\n";
	}
